...
/**
 * Mint JitoSOL from treasury SOL, then Kamino single-sided deposit — **no** StakeToCurate ix.
 * Used by the vault-yield worker after surplus SOL lands in the treasury account.
 */
export async function runTreasuryPoolCompound(params: {
  connection: Connection
  wallet: YieldWallet
  /** SOL to route through JitoSOL → Kamino (lamports). */
  lamports: bigint
  /** Defaults to app env / canonical mainnet strategy. */
  kaminoStrategy?: PublicKey
}): Promise<void> {
  const { connection, wallet, lamports } = params
  if (lamports > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error('Amount too large for this client path')
  }
  const strategyPk =
    params.kaminoStrategy ?? kaminoJitosolSolStrategyFromEnv()
  ...
  const strategyAddr = address(strategyPk.toBase58())
  ...
  if (lookupTables.length > 0) {
    ...
    await sendAndConfirm(connection, legacySigningWallet(wallet), legacyKamino)
  }
}
